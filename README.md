...
&#160; &#160; &#160; &#160;(5).**Ensemble(模型融合)。** 集成学习算法也可以有效的减轻过拟合，常见的有Bagging和Boosting，通过结合几个模型降低泛化误差。Bagging主要思想是通过对训练数据有放回的采样来训练多个模型，Boosting主要思想是通过将弱分类器组装成一个强分类器。测试时，让所有模型表决，Bagging每个模型表决权值一样，Boosting中分类误差小的模型表决权值大，分类误差大的模型表决权值小，对于回归思想类似。模型融合虽然在准确率和稳定性上都会有提高，但是训练多个模型的计算成本是非常高的。

&#160; &#160; &#160; &#160;(6).**Dropout(随机失活)。** 假设我们选取一个隐层，在每一次梯度迭代中，让隐层的每一个单元以一定的概率失活，这相当于训练一个新的子网络，下一次迭代继续上述过程。Dropout的底层思想其实也是把多个模型的结果放到一起，只不过实现方式是在一个模型内部，而不是像Bagging或Boosting一样，真正地去训练多个模型。定性地理解一下，dropout的实现相当于每次迭代都在一个随机挑选的特征子空间进行。在实现dropout的时候，因为训练时对单元按照一定概率p进行随机置零，所以训练结束后到了预测阶段需要对参数的值乘上一个1-p，才能保证结果的正确性。
